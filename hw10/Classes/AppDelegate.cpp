...
bool AppDelegate::applicationDidFinishLaunching() {
	...
	legAnimation->setDelayPerUnit(0.1f);
	AnimationCache::getInstance()->addAnimation(legAnimation, "legAnimation");

	//miner mouth
	char mframeName[40];
	Animation* mouthAnimation = Animation::create();
	for (int i = 0; i < totalFrames; i++) {
		sprintf(mframeName, "miner-face-whistle-%d.png", i);
		mouthAnimation->addSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(mframeName));
	}

	mouthAnimation->setDelayPerUnit(0.1f);
	AnimationCache::getInstance()->addAnimation(mouthAnimation, "mouthAnimation");

	//mouse 
	char mouseFrameName[100];
	...
	mouseAnimation->setDelayPerUnit(0.1f);
	AnimationCache::getInstance()->addAnimation(mouseAnimation, "mouseAnimation");

	//cave
	char caveFrameName[100];
	Animation* caveAnimation = Animation::create();
	for (int i = 0; i < 3; i++) {
		sprintf(caveFrameName, "cave-%d.png", i);
		caveAnimation->addSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(caveFrameName));
	}
	caveAnimation->setDelayPerUnit(0.1f);
