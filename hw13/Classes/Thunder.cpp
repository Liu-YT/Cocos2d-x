...
void Thunder::newEnemy() {
	...
	float enemyWidth = visibleSize.width / 6.0;

	//先前的向下移动
	for (auto enemy : enemys) {
		enemy->runAction(MoveBy::create(0.0, Point(0, -50)));
	}
	...
	char enemyPath[20];
	sprintf(enemyPath, "stone%d.png", 3 - i);
	double width = enemyWidth, height = visibleSize.height - 50;
	for (int j = 0; j < 5; ++j) {
		auto enemy = Sprite::create(enemyPath);
		enemy->setAnchorPoint(Vec2(0.5, 0.5));
		enemy->setScale(0.5, 0.5);
		enemy->setPosition(width * (j + 1) + moveDir, height);
		enemys.push_back(enemy);
		addChild(enemy, 1);
	}
}
...
void Thunder::movePlane(char c) {
	...
	log("%f %f", x, y);
	if (c == 'A' && x - 5 >= player->getContentSize().width && isMove)
		player->runAction(MoveBy::create(0.2, Point(-5, 0)));
	if(c == 'D' && x + 5 <= visibleSize.width - player->getContentSize().width && isMove)
		player->runAction(MoveBy::create(0.2, Point(5, 0)));
}
...
void Thunder::fire() {
	...
	// 移除飞出屏幕外的子弹
	// Todo
	auto move = MoveBy::create(1.5f, Vec2(0, visibleSize.height));
	Sequence* seq = Sequence::create(move, CallFunc::create([bullet, this] {
...
void Thunder::explosion() {
	...
	explore.reserve(8);
	for (int i = 0; i < 8; i++) {
		auto frame = SpriteFrame::createWithTexture(texture2, CC_RECT_PIXELS_TO_POINTS(Rect(180 * i, 0, 180, 160)));
...
void Thunder::meet(EventCustom * event) {
	...
	//一次只能爆炸一个，否则remove后将报错
	for (auto bullet : bullets) {
		bool flag = false;	//是否有过命中的
		for (auto enemy : enemys) {
			if (bullet->getPosition().getDistance(enemy->getPosition()) < 25) {
				
				auto explose = Animate::create(Animation::createWithSpriteFrames(explore, 0.05f, 1));
...
void Thunder::onTouchMoved(Touch *touch, Event *event) {
	...
	if (isClick) {
		...
		delta.y = 0;
		if(player->getPosition().x + delta.x >= player->getContentSize().width && player->getPosition().x + delta.x <= visibleSize.width - player->getContentSize().width)
			player->runAction(MoveBy::create(0.0, delta));
	}
}
